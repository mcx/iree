...
static bool isFusableWithConsumer(OpOperand &use) {
  ...
  if (isa<linalg::LinalgOp>(producer) && isa<linalg::LinalgOp>(consumer) &&
      areFusableLinalgOps(use)) {
    return true;
  }

  // Fuse pad op with its producer.
  if (isa<tensor::PadOp>(consumer) && isa<TilingInterface>(producer)) {
    return true;
  }
...
static bool isFusableWithProducer(OpOperand &operand) {
  ...
  if (isa<tensor::PadOp>(consumer) && isa<linalg::LinalgOp>(producer)) {
    return true;
  }
  return false;
}
