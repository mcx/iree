...
bool AsyncUpdateOp::isMetadata() {
  // For now we are very conservative: if there are any other users of the
  // update value we don't in-place it and treat it as a copy.
  // We could do some more sophisticated rewrites to replace other users with
  // a slice of the target instead and avoid the copy.
  if (getUpdate().hasOneUse()) {
    // Only user of the update value is this - can allocate in-place.
    return true;
  }
  // Multiple users - perform copies.
  return false;
}
