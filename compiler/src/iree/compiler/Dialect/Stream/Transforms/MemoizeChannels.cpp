...
class MemoizeChannelsPass : public MemoizeChannelsBase<MemoizeChannelsPass> {
 public:
  ...
  void runOnOperation() override {
    ...
    SmallVector<Attribute, 4> defaultKeys;
    ...
    for (auto callableOp : moduleOp.getOps<CallableOpInterface>()) {
      callableOp.walk([&](IREE::Stream::ChannelDefaultOp defaultOp) {
        ...
        if (lookup.second) {
          defaultKeys.push_back(std::move(fullKey));
        }
    ...
    for (auto defaultKey : llvm::enumerate(defaultKeys)) {
      auto defaultOps = allDefaultOps[defaultKey.value()];
      auto anyDefaultOp = defaultOps.front();
      auto affinityAttr = IREE::Stream::AffinityAttr::lookup(anyDefaultOp);
      ...
      std::string variableName =
          "_channel_" + std::to_string(defaultKey.index());
